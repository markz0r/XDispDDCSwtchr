...
func (e *Engine) monitorModel(monitorID string) (*config.Model, error) {
	...
	if monitorID != "" {
		for _, m := range e.cfg.Monitors { if m.Identifier == monitorID { modelName = m.Model; break } }
	} else if len(e.cfg.Monitors) > 0 { modelName = e.cfg.Monitors[0].Model }
	if modelName == "" { return nil, fmt.Errorf("no monitor/model match for '%s'", monitorID) }
	for i := range e.cfg.Models { if e.cfg.Models[i].Name == modelName { return &e.cfg.Models[i], nil } }
...
func (e *Engine) setInputSource(monitorID, sourceName string) error {
	...
	if !ok { return fmt.Errorf("unknown input '%s' for model %s", sourceName, md.Name) }
	return e.backend.SetVCP(monitorID, md.InputSelect.Code, val)
}

func (e *Engine) nextInputSource(monitorID string) error {
	md, err := e.monitorModel(monitorID); if err != nil { return err }
	keys := make([]string, 0, len(md.InputSelect.Values))
	for k := range md.InputSelect.Values { keys = append(keys, k) }
	sort.Strings(keys)
	if len(keys) == 0 { return fmt.Errorf("no inputs configured") }
...
func (e *Engine) togglePIP(monitorID string) error {
	md, err := e.monitorModel(monitorID); if err != nil { return err }
	if md.PIP == nil { return fmt.Errorf("PIP not configured for model %s", md.Name) }
	on := e.bumpIndex(monitorID, "pip_on", 2) == 1
	val := md.PIP.OffValue
	if on { val = md.PIP.OnValue }
	return e.backend.SetVCP(monitorID, md.PIP.ToggleCode, val)
}

func (e *Engine) nextPIPSource(monitorID string) error {
	md, err := e.monitorModel(monitorID); if err != nil { return err }
	if md.PIP == nil || md.PIP.SourceCode == "" || len(md.PIP.Sources) == 0 { return fmt.Errorf("PIP sources not configured") }
...
func (e *Engine) bumpIndex(monitorID, feature string, n int) int {
	if e.iterState[monitorID] == nil { e.iterState[monitorID] = map[string]int{} }
...
func (e *Engine) lookupNamed(md *config.Model, key string) (uint16, bool) {
	if md.VCPNamedValues == nil { return 0, false }
...
func (e *Engine) Dispatch(action string, args map[string]string, target string) error {
	...
	case "setinputsource":
		src := args["source"]
		if src == "" { return fmt.Errorf("missing args.source") }
